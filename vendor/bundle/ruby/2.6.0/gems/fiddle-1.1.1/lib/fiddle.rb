...
module Fiddle
  ...
  if defined?(Fiddle::Types)
    Fiddle::Types.constants.each do |type|
      const_set "TYPE_#{type}", Fiddle::Types.const_get(type)
    end
  end

  # Add missing unsigned TYPE_* constants for Ruby 2.6 system fiddle compatibility
  # (system fiddle.bundle defines signed variants but not unsigned)
  unless defined?(TYPE_UCHAR)
