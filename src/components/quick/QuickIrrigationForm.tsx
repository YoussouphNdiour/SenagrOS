...
export function QuickIrrigationForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      status: 'done',
      data: {
        duration_min: dureeMin ? Number(dureeMin) : undefined,
        volume_liters: volumeLitres ? Number(volumeLitres) : undefined,
      },
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <h1 className="mb-6 text-xl font-semibold text-gray-800">{t('titleIrrigation')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t('fieldDureeMin')}</label>
            <input
              type="number"
              min="0"
              step="1"
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t('fieldVolumeLitres')}</label>
            <input
              type="number"
              min="0"
              step="0.1"
        ...
        >
          {isPending ? t('submitting') : t('submitIrrigation')}
        </button>
      </form>
    </div>
  );
