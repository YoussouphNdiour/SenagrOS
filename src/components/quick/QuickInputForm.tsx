...
export function QuickInputForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      type: 'input',
      name: produit ? `Intrant — ${produit}` : 'Intrant rapide',
      timestamp: new Date().toISOString(),
      status: 'done',
      data: {
        input_type: 'phyto',
        dose: dose ? Number(dose) : undefined,
        dose_unit: unite || undefined,
      },
  ...
  return (
    <div className="mx-auto max-w-lg">
      ...
      <h1 className="mb-6 text-xl font-semibold text-gray-800">{t('titleInput')}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 rounded-xl bg-white p-6 shadow-sm">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">{t('fieldProduit')}</label>
          <input
            type="text"
            value={produit}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t('fieldDose')}</label>
            <input
              type="number"
              min="0"
              step="0.01"
          ...
          <div>
            <label className="mb-1 block text-sm font-medium text-gray-700">{t('fieldUnite')}</label>
            <input
              type="text"
              value={unite}
        ...
        >
          {isPending ? t('submitting') : t('submitInput')}
        </button>
      </form>
    </div>
  );
